          ...
          <OverlayTrigger
            ...
            overlay={
              <Popover id={styles['target-definition']}>
                ...
                <Popover.Body className={styles.scroll}>{targetWord.definitions.map((targetDefs, index) => <li key={index}><span className='fst-italic'>{targetDefs[0]}</span> - {targetDefs[1]}</li>)}</Popover.Body>
              </Popover>
            }
          >
            <Card.Header
              as='h2'
              className={`${wordDifficulty} ${styles.pointer} fw-semibold`}
            >{targetWord.word}
            </Card.Header>
          </OverlayTrigger>)}
      <Card.Body>
